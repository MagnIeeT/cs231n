...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type=="rnn":
            dword_emb, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, cache_rnn)
        elif self.cell_type=="lstm":
            dword_emb, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh, cache_lstm)
        else:
            print("Wrong cell type")
        grads['W_embed']                                     = word_embedding_backward(dword_emb,cache_embed)
    ...
    def sample(self, features, max_length=30):
        ...
        ind = self._start*np.ones((N,1),dtype=np.int32)
        c = np.zeros_like(h)
        for i in range(max_length):
            captions[:,i]= ind.reshape(N,)
            x,_ = word_embedding_forward(ind,W_embed)
            x = x.reshape(N,-1)
            if self.cell_type=='rnn':
