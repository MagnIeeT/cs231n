...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    p = np.exp(scores)/np.sum(np.exp(scores),keepdims=True, axis=1)
    ...
    #############################################################################
    p_grad = p.copy()
    p_grad[np.arange(N),y] -= 1
    
    grads['b2']= np.sum(p_grad,axis=0)/N
    grads['W2']= (h1.T@p_grad)/N + 2*reg*W2
    
    h1_grad = p_grad@W2.T
    h1_grad_max = h1_grad*(h1>0)
    
    grads['b1'] = np.sum(h1_grad_max,axis=0)/N
    grads['W1'] = (X.T@h1_grad_max)/N + 2*reg*W1
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
